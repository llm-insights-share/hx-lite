...
- 自动触发 adapter 投影
- `-g, --global`：Skill / Command 装到用户级目录（Cursor `~/.cursor/skills`、`~/.cursor/commands`；Trae `~/.trae/skills`；Trae 中国版 `trae-cn` → `~/.trae-cn/skills`）。`--local` 强制项目级（`.cursor/`、`.trae/` / `.trae-cn/`）。默认项目级。Hooks 始终写在项目 `.cursor/hooks/`。
...
- `--global` 只写用户级目录，不写项目 `.cursor/skills` / `.trae/skills`（项目级会覆盖同名全局 Skill）
...
.cursor/hooks/nhx-*.mjs          # hooks 始终项目级
.trae/skills/nhx-*/
