...
nhx init --project 1 --stages req --targets cursor,trae --global
...
- 自动触发 adapter 投影
- `-g, --global`：Skill / Command 装到用户级目录（Cursor `~/.cursor/skills`、`~/.cursor/commands`；Trae `~/.trae/skills`；Trae 中国版 `trae-cn` → `~/.trae-cn/skills`）。`--local` 强制项目级（`.cursor/`、`.trae/` / `.trae-cn/`）。默认项目级。Hooks 始终写在项目 `.cursor/hooks/` 与 `.trae/hooks/`（Trae-CN 亦读取项目 `.trae/hooks.json`）。
- 支持的 `--targets`：`cursor`、`trae`、`trae-cn`（未知目标会告警并跳过）
...
- `--global` 只写用户级目录，不写项目 `.cursor/skills` / `.trae/skills`（项目级会覆盖同名全局 Skill）；Hooks 仍写项目 `.cursor/hooks/`、`.trae/hooks/`
...
nhx session mark --from-prompt "/nhx-req-prd-writing" --ide trae-cn
```

- 记录当前任务上下文，供 hooks/检查流程读取；可选 `--ide cursor|trae|trae-cn`（默认 `cursor`）
