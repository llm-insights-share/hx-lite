...
| `nhx` | 从 WebUI 按 stage 拉资产 → 投影到 Cursor/Trae → Sensor 检查 → 提交产物 / 发起人工审批 |
...
组织 HX
  Profile → Stage → Task
              ...
              └─ Sensors（反馈：rules / shell / inline / human）
                    ↑
              Task 壳（Command Shell / Skill Shell，壳编辑器）
        ↓ 项目「初始化配置」
项目 HX（可再自定义 Guide / Sensor / Task）
        ↓ nhx init / sync
本地 .nhx/ + IDE 命令壳 nhx-*
```
...
1. **工具 → 初始配置生成**：一键 bootstrap（Profile / Stage-Task / Guide / Sensor / Command）。
...
| Stage & Task | 维护任务矩阵；**直接多选 Guide / Sensor**；支持编辑绑定 |
| Guide & Sensor | 维护资产；Sensor 的 Check Type：`rules` / `shell` / `inline` / **`human`** |
| 壳编辑器 | Task 壳（同时投影 Command Shell + Skill Shell） |

**Stage & Task 编辑建议：**

1. 打开目标 Task →「编辑」。
2. 在 **Sensor 资产** 勾选关卡（人工关卡如 `prd-approved`，列表中会标紫色）。
3. 保存。项目侧需「重新初始化」或本地 `nhx sync` 才会吃到变更。

### 3.3 Sensor Check Type
...
| `cli` | nhx 人工指令 | `nhx sensor check`（默认） |
| `task-shell` | command/skill 壳 | 任务壳附录要求执行 check |

创建 Sensor 时「配置内容」只填 **check 专属字段**（如 `expr` / `rules_text` / bash 块）；**触发通道与 Scope 只在表单**配置，不要写入 content。选 `inline` 时可点击内置函数列表插入 `expr`。
...
| dev | `harnessX/changes/<id>/` 或 `openspec/changes/<id>/`（shell 传感器会解析 `HX_CHANGE` / 最新目录） |
| test | `docs/test/*` |
...
2. 点 **初始化配置**：从组织按 Profile 拉 Stage/Task，并复制组织中的 **全部 Guide、Sensor**，保留任务绑定。
...
| Guide 管理 | 项目级 Guide |
| Sensor 管理 | 项目级 Sensor；可建/编辑 `human` 类型 |
| 自定义 Task | 新建自定义任务，或 **编辑** profile/custom 任务的 Guide/Sensor 绑定 |
| 审批工单 | 创建/提交/批准/驳回；人工关卡用类型 **`human-check`**，必填 Stage/Task |

### 4.3 自定义 Task

- 创建时可多选 Guide、Sensor（标签含 `· human` 即为人工关）。
...
2. 创建 → **提交** → 审批人 **批准**。
3. 仅 **已批准** 的 `human-check` 会使对应 human Sensor 通过（其它类型工单不计入）。
...
| `nhx sensor check [--stage --task]` | 跑当前任务绑定的 Sensor |
...
.cursor/hooks/nhx-sensor-stop.mjs
...
| `stop` | `nhx-sensor-stop.mjs` | 跑 `nhx sensor check`；失败则 `followup_message` 追问 |

`nhx sensor check` 对 `human` 会请求：
...
1. **组织 → Guide & Sensor**  
   - 使用默认 `prd-approved`（已是 `human`），或新建 Sensor，Check Type = `human`。
2. **组织 → Stage & Task**  
   - 编辑目标 Task（如 `req` / `prd-writing`），勾选该 Sensor。
3. **项目 → 项目列表 → 重新初始化配置**  
   - 或本地已 init 过则执行 `nhx sync`。
4. 开发者完成产物后：  
   `nhx approve request --stage req --task prd-writing`
5. 审批人在 **项目 → 审批工单** 批准。
6. `nhx sensor check` / Cursor stop hook 对应该关卡 **PASS**。

项目侧也可在 **Sensor 管理** 建 human Sensor，再在 **自定义 Task** 编辑绑定，不必改组织矩阵。
...
| 角色 | WebUI | nhx |
|------|-------|-----|
| 组织管理员 | bootstrap、矩阵、Guide/Sensor、GitHub 推送 | — |
...
| human Sensor 一直 FAIL | 确认工单类型为 `human-check`、Stage/Task 完全一致、状态为 `approved` |
...
- [ ] WebUI 登录；组织 bootstrap；Stage & Task 能编辑 Sensor 绑定  
- [ ] 新建/编辑 Sensor，`check_type=human` 显示紫色标签  
...
- [ ] `nhx approve request` → WebUI 批准 → `nhx sensor check` PASS  
