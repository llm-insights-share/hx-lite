...
- **Zhang** (architect): human approver for design→plan (CODEOWNERS owner of `harnessX/changes/**/specs/`).
...
4. Run `hx gate check partial-refund --stage dev --task propose` — do not finish until green.
...
### 3. dev stage advancement: propose → design → design-to-plan approval → plan
...
- **Spec-first meaning**: before code, agent Context Pack (`hx guide pack --stage dev --task apply`) contains only this change's design/tasks/delta spec + constitution + apply Skills — **no noise from other changes** — feedforward for context quality.
- **Human approval approves intent**: machines verify implementation (tests, lint, architecture); humans confirm "we are building this" only at design→plan. Approval carries artifact hash; post-hoc spec edits are detected.
