...
agent 按 `/hx-dev-propose` 薄清单起草：proposal.md 三句话写完；delta spec 只有一个 ADDED Requirement（写完自跑 `hx gate check` 确认格式过关）：
...
事故场景下李工选择留在 Cursor 会话里盯着做（`/hx-dev-apply hotfix-expired-coupon-500`，agent 先补红测试再写修复，每步跑 fast-lite 套件），也可以挂 headless 循环：
