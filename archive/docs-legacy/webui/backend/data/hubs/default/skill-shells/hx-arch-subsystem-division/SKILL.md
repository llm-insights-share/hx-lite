---
name: hx-arch-subsystem-division
description: 子系统划分
---
# /hx-arch-subsystem-division — 子系统划分
...
| id | kind | source |
|----|------|--------|
| `arch-authoring` | guide.skill | |

### 绑定 Templates

| id | kind | source |
|----|------|--------|
| `arch-hld-template` | guide.template | |

### 如何使用绑定 Guide
- **Skill：** 遵循 `arch-authoring`。
- **Template：** 使用 `arch-hld-template` 组织交付物结构。
...
### 特别约束 — 门禁
宣称完成前：执行 arch check `--task subsystem-division`，未通过不得结束。
