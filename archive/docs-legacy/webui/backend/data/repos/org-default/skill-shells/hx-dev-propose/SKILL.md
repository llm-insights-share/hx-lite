...
### 特别上下文 — 加载上下文
1. 加载阶段 `dev` 任务 `propose` 的 Context Pack。
...
| id | kind | source |
|----|------|--------|
| `proposal-template` | guide.template | |
| `requirements-template` | guide.template | |
...
### 特别约束 — 门禁
宣称完成前：执行 gate check `<change> --stage dev --task propose`，未通过不得结束。
