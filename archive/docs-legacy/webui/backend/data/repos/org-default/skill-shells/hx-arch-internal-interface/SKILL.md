---
name: hx-arch-internal-interface
description: 内部接口设计
---
...
| id | kind | source |
|----|------|--------|
| `arch-module-boundary` | guide.skill | |

### 绑定 Templates

| id | kind | source |
|----|------|--------|
| `arch-lld-template` | guide.template | |

### 如何使用绑定 Guide
- **Skill：** 遵循 `arch-module-boundary`。
- **Template：** 使用 `arch-lld-template` 组织交付物结构。
...
### 特别约束 — 门禁
宣称完成前：执行 arch check `--task internal-interface`，未通过不得结束。
