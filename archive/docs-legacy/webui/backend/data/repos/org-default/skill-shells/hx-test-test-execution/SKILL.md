...
### 特别上下文 — 加载上下文
1. 加载阶段 `test` 任务 `test-execution` 的 Context Pack。
...
| id | kind | source |
|----|------|--------|
| `test-execution` | guide.skill | |
| `uat-checklist` | guide.skill | |
...
### 特别约束 — 门禁
宣称完成前：执行 gate check `<change> --stage test --task test-execution`，未通过不得结束。
