...
- Qoder 首次需在 IDE 设置中确认启用 Hook（项目级 `.qoder/settings.json`；全局 `~/.qoder/settings.json` 或 `$QODER_CONFIG_DIR/settings.json`）
...
- 记录当前任务上下文，供 hooks/检查流程读取；可选 `--ide cursor|trae|trae-cn|codebuddy|workbuddy|qoder`（默认 `cursor`）
...
.qoder/settings.json             # Qoder hooks 合并写入
...
| Qoder 命令未出现 | 确认已 `nhx adapter sync --targets qoder`；命令文件以 `---` frontmatter 开头；执行 `/commands` 重新加载 |
