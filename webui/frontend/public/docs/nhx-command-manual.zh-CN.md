...
nhx init --project demo-proj --stages req --targets cursor,trae,codebuddy
nhx init --project 1 --stages req --targets cursor,trae,codebuddy --global
...
- 自动触发 adapter 投影
- `-g, --global`：Skill / Command 装到用户级目录（Cursor `~/.cursor/skills`、`~/.cursor/commands`；Trae `~/.trae/skills`；Trae 中国版 `trae-cn` → `~/.trae-cn/skills`；CodeBuddy `~/.codebuddy/skills`、`~/.codebuddy/commands`；WorkBuddy `~/.workbuddy/skills`、`~/.workbuddy/commands`）。`--local` 强制项目级（`.cursor/`、`.trae/` / `.trae-cn/`、`.codebuddy/`）。默认项目级。
- 支持的 `--targets`：`cursor`、`trae`、`trae-cn`、`codebuddy`、`workbuddy`（未知目标会告警并跳过）
...
- `--global` 只写用户级目录，不写项目 `.cursor/skills` / `.trae/skills` / `.codebuddy/skills`（项目级会覆盖同名全局 Skill）
...
- 记录当前任务上下文，供 hooks/检查流程读取；可选 `--ide cursor|trae|trae-cn|codebuddy|workbuddy`（默认 `cursor`）
